...
import { verifyEmailOtp, normalizeEmail, normalizeOtp, describeOtpError } from '@/lib/otp';
...
const input = z.object({
  ...
  // Strip anything that isn't a digit before length-checking — mobile
  // copy/paste from an email client very commonly grabs a stray space,
  // line break, or surrounding words along with the code, and the client
  // may not always sanitize it first. Without this, a code that is
  // otherwise still valid gets rejected and reads to the user as
  // "invalid or expired" even though it hasn't expired.
  code: z.string().transform(normalizeOtp).pipe(z.string().length(6)),
});
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message = details.code === 'otp_expired'
      ? 'That verification code is no longer valid. Please request a new code.'
      : details.status === 429 || details.code === 'over_request_rate_limit' || details.code === 'over_email_send_rate_limit'
        ? 'Too many verification attempts. Please wait and try again.'
        : 'That verification code is invalid. Please check the code and try again.';
    return NextResponse.json({ error: message }, { status: details.status === 429 ? 429 : 400 });
  }
  ...
  if (!user) {
    ...
    try {
      ...
      // The Supabase OTP has been consumed at this point. Remove the just-created
      // Auth identity so a transient database failure does not strand the email
      // in a half-registered state and prevent the user from trying again.
      await deleteSupabaseAuthUser(authUser.id).catch(cleanupError =>
        console.error('[otp/verify] Supabase cleanup failed:', cleanupError instanceof Error ? cleanupError.message : cleanupError),
      );
      return NextResponse.json({ error: 'Could not finish creating your account. Nothing was charged or lost. Please request a new code and try again.' }, { status: 500 });
    }
