...
export async function POST(request: Request) {
  ...
  if (d.mode === 'register') {
    ...
    if (d.phone) {
      const phoneOwner = await prisma.user.findUnique({ where: { phone: d.phone }, select: { id: true, email: true, status: true } }).catch((err) => {
        console.error('[otp/send] phone lookup failed:', err instanceof Error ? err.message : err);
        return null;
      });
      if (phoneOwner && phoneOwner.email !== d.email && phoneOwner.status !== 'DELETED') {
        return NextResponse.json({ error: 'That phone number is already registered to another account. Use a different number, or contact support if this is your number.' }, { status: 409 });
      }
    }
