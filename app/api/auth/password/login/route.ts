...
export async function POST(request: Request) {
  try {
    ...
    if (needsEmailRevalidation) { try { await sendEmailOtp(user.email, { shouldCreateUser: false }); } catch {} const response = NextResponse.json({ requiresEmailVerification: true }); response.cookies.set('tp_device', device.token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: 31536000 }); return response; }
    ...
    console.error('[login] password sign-in failed:', error instanceof Error ? error.message : error);
