...
export default async function Home() {
  const [categories, verifiedCount, completedJobsCount, customerCount, featuredProfessionals] = await Promise.all([
  ...
  return (
    <main>
      ...
      {featuredProfessionals.length > 0 && (
        <section className="landing">
          <div className="overview-top reveal">
            <div>
              <p className="eyebrow">MEET THE PROS</p>
              <h2 className="section-title">Top-rated professionals</h2>
            </div>
            <a className="primary" href="/marketplace">See all professionals →</a>
          </div>
          <div className="professional-grid">
            {featuredProfessionals.map((p, i) => (
              <Link key={p.id} href={'/marketplace/' + p.id} className="professional-card reveal" style={{ transitionDelay: `${i * 60}ms` }}>
