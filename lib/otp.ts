...
export function missingOtpEnvVars(): string[] {
  return [
    ...
    !(process.env.SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) && 'SUPABASE_ANON_KEY',
...
/** Normalize OTP input for mobile copy/paste/autofill. Supabase remains the
 * authority for validity and single-use; this only removes presentation
 * whitespace/non-digit characters before sending the token to Supabase. */
export function normalizeOtp(token: string): string {
  return token.replace(/\D/g, '').slice(0, 10);
}
