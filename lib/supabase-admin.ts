...
export async function createSupabaseAuthUser(params: { email: string; password?: string; emailConfirm?: boolean; userMetadata?: Record<string, unknown> }) {
  ...
  if (error || !data.user) throw error ?? new Error('Could not create the Supabase Auth user.');
  return data.user;
}
