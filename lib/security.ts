...
const PASSWORD_MAXMEM = 128 * 1024 * 1024;
...
export function hashPassword(password: string) {
  ...
  const derived = scryptSync(password, salt, PASSWORD_KEYLEN, { N: PASSWORD_COST, r: PASSWORD_BLOCK_SIZE, p: PASSWORD_PARALLELIZATION, maxmem: PASSWORD_MAXMEM });
...
export function verifyPassword(password: string, stored: string) {
  try {
    ...
    const actual = scryptSync(password, salt, expected.length, { N: PASSWORD_COST, r: PASSWORD_BLOCK_SIZE, p: PASSWORD_PARALLELIZATION, maxmem: PASSWORD_MAXMEM });
...
export async function ensureDevice(userId: string, token: string, request: Request) {
  ...
  let deviceToken = token || createDeviceToken();
  let tokenHash = deviceHash(deviceToken);
  ...
  if (existing && existing.userId === userId && !existing.revokedAt) {
    ...
    return { id: existing.id, isNew: false, token: deviceToken };
  }
  // A stale cookie can belong to another account (or a revoked device). Never
  // reuse that tokenHash for a different user; issue a fresh device token.
  if (existing) {
    deviceToken = createDeviceToken();
    tokenHash = deviceHash(deviceToken);
  }
  ...
  return { id: created.id, isNew: true, token: deviceToken };
}
