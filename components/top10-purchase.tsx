...
export function Top10Purchase({ availableBalance }: { availableBalance: number }) {
  ...
  return <div className="growth-purchase-box"><div className="growth-balance"><span>MVault available</span><strong>₦{(availableBalance/100).toLocaleString('en-NG')}</strong></div>{listings.length>0&&<div className="growth-active"><span>● TOP 10 ACTIVE</span><strong>Until {new Intl.DateTimeFormat('en-NG',{dateStyle:'medium'}).format(new Date(listings[0].expiresAt))}</strong></div>}<div className="advert-plan-grid">{products.map(p=><button key={p.id} type="button" className={`advert-plan ${selected===p.id?'selected':''}`} onClick={()=>setSelected(p.id)}><span>{p.durationDays} days</span><strong>₦{(p.price/100).toLocaleString('en-NG')}</strong><small>MVault payment</small></button>)}</div>{error&&<p className="growth-error">{error}</p>}<button type="button" className="growth-buy-button" onClick={buy} disabled={!selected||busy}>{busy?'Activating…':'Get Top 10 Placement →'}</button><small className="growth-payment-note">Top 10 placement uses your MVault balance and does not change Score, Level or organic ranking.</small></div>;
}
