...
type MVault = { id:string; availableBalance:number; pendingBalance:number; professional:{fullName:string;user:{email:string}}; dedicatedAccount?:{accountNumber?:string|null;bankName?:string|null;status:string;active:boolean}|null };
...
export function AdminFinance() {
  ...
  const [wallets,setMVaults]=useState<MVault[]>([]);
  ...
  async function load() {
    ...
    try {
      ...
      setWithdrawals(wb.withdrawals??[]); setTransactions(tb.transactions??[]); setTransfers(ib.transfers??[]); setAccounts(ib.accounts??[]); setMVaults(wbx.wallets??[]); setPage(1);
  ...
  return <div className="finance-control-center">
    ...
    {rejectId&&<div className="finance-modal-backdrop"><div className="finance-modal"><h3>Reject withdrawal</h3><p>The reserved MVault funds will be returned to the user's available balance.</p><textarea value={rejectReason} onChange={e=>setRejectReason(e.target.value)} placeholder="Reason for rejection"/><div><button className="finance-btn" onClick={()=>setRejectId(null)}>Cancel</button><button className="finance-btn danger" disabled={!rejectReason.trim()} onClick={async()=>{const id=rejectId;setRejectId(null);await action(id,'reject',{reason:rejectReason});}}>Reject & return funds</button></div></div></div>}
