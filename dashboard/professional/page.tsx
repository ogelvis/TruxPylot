...
import { getProfessionalActions } from '@/lib/next-actions';
...
export default async function ProfessionalDashboard() {
  ...
  const recommendations = getProfessionalActions({
    verificationStatus: professional.verificationStatus,
    unansweredEnquiries,
    awaitingQuotes,
    unreadMessages,
    servicesCount: professional.services.length,
    portfolioItemsCount: professional.portfolioItems.length,
    reviewsCount: reviews,
    eligibleForPublicScore: Boolean(score?.eligibleForPublicScore),
    top10ExpiresAt: activeTop10?.expiresAt ?? null,
    advertExpiresAt: activeAdvert?.expiresAt ?? null,
  });
  ...
  return <AppShell role="PROFESSIONAL" name={displayName} avatarUrl={professional.avatarUrl} verified={professional.verificationStatus === 'APPROVED'} active="/dashboard/professional" isBusiness={professional.accountType === 'BUSINESS' || Boolean(teamMembership)}>
    <main className="dash-page"><div className="overview-top"><div><h1>Business Command Center</h1><p className="subcopy">A live view of your business activity and next best actions.</p></div><a className="primary" href={`/marketplace/${professional.id}`}>View public profile →</a></div>
      ...
      {recommendations.length > 0 && <section className="panel recommendations"><div className="panel-head"><h2>Recommended next steps</h2></div><ul>{recommendations.map(action => <li key={action.id} className={`urgency-${action.urgency}`}><a href={action.href}>{action.label}</a></li>)}</ul></section>}
