...
export function WalletActions({ availableBalance, payoutAccount, withdrawOnly=false, section }:WalletActionsProps){
  ...
  function withdraw(){const amountInKobo=Math.round(Number(withdrawAmount)*100);setWithdrawError('');if(!Number.isFinite(amountInKobo)||amountInKobo<100000){setWithdrawError('Minimum withdrawal is ₦1,000.');return;}if(amountInKobo>availableBalance){setWithdrawError('Insufficient wallet balance.');return;}if(!payoutAccount||!payoutAccount.verified){setWithdrawError('Verify your payout bank account before withdrawing.');return;}setConfirmOpen(true);}
