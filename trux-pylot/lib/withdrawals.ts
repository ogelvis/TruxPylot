...
export async function processWalletWithdrawal(withdrawalId: string, adminUserId: string) {
  ...
  try {
    ...
    await prisma.withdrawal.update({ where: { id: withdrawal.id }, data: { status: 'APPROVED', providerTransferId: String(transfer.id), providerTransferCode: transfer.transfer_code ?? null, providerStatus: status || 'pending', providerError: transfer.failures ? JSON.stringify(transfer.failures) : null } });
...
export async function finalizeWalletWithdrawal(withdrawalId: string, otp: string, adminUserId: string) {
  ...
  await prisma.withdrawal.update({ where: { id: withdrawal.id }, data: { status: 'APPROVED', providerTransferId: String(transfer.id), providerStatus: status, providerError: transfer.failures ? JSON.stringify(transfer.failures) : null } });
...
export async function markWithdrawalSuccessful(withdrawalId: string, providerTransferId?: number | string, transferCode?: string, providerStatus = 'success') {
  await prisma.$transaction(async tx => {
    ...
    if (transaction) await tx.walletTransaction.update({ where: { id: transaction.id }, data: { status: 'COMPLETED', description: 'Withdrawal paid to bank account', metadata: { withdrawalId: withdrawal.id, providerTransferId, providerStatus } } });
  });
}
