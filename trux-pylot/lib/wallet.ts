...
async function paystackWalletRequest(path: string) {
...
/**
 * Reconciles successful Paystack transactions for an existing DVA directly
 * from Paystack. This is a recovery path for delayed/missed webhooks; it does
 * not replace the webhook and is idempotent through the existing wallet ledger.
 */
export async function reconcileDedicatedAccountTransfers(dedicatedAccountId: string) {
  const account = await prisma.dedicatedAccount.findUnique({
    where: { id: dedicatedAccountId },
  });
  if (!account) return { checked: 0, credited: 0, unmatched: 0 };

  const customer = await paystackWalletRequest(`/customer/${encodeURIComponent(account.paystackCustomerCode)}`);
  const customerId = customer?.id ?? customer?.customer?.id;
  if (customerId == null) return { checked: 0, credited: 0, unmatched: 0 };
  ...
  const data = await paystackWalletRequest(`/transaction?${query.toString()}`);
  ...
  for (const transaction of transactions) {
    const receiver = transaction?.authorization?.receiver_bank_account_number;
    const channel = transaction?.authorization?.channel ?? transaction?.channel;
    const isThisDva = String(receiver ?? '') === String(account.accountNumber ?? '') && Boolean(account.accountNumber);
    const isDedicatedChannel = channel === 'dedicated_nuban';
    if (!isThisDva && !isDedicatedChannel) continue;

    checked += 1;
    const result = await applyDedicatedAccountTransfer({ event: 'charge.success', data: transaction });
    if (result.matched && !result.duplicate) credited += 1;
    if (!result.matched) unmatched += 1;
  }

  console.info('[DVA RECONCILIATION]', {
    dedicatedAccountId,
  ...
  return { checked, credited, unmatched };
}
